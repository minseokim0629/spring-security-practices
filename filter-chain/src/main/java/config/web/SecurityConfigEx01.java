		...
		List<SecurityFilterChain> securityFilterChains = Arrays.asList(
				new SecurityFilterChain() {
					...
					public boolean matches(HttpServletRequest request) {
						String uri = request.getRequestURI().replaceAll(request.getContextPath(), ""); // contextpath가 빠진 uri 리턴 ex) /hello, /assets/...
						return new AntPathMatcher().match("/hello/**", uri); // 두가지 방법 존재 1. ant 2. 정규표현식
					}
				...
				new SecurityFilterChain() {
					...
					public boolean matches(HttpServletRequest request) {
						String uri = request.getRequestURI().replaceAll(request.getContextPath(), ""); // contextpath가 빠진 uri 리턴 ex) /hello, /assets/...
						return new AntPathMatcher().match("/ping/**", uri); // 두가지 방법 존재 1. ant 2. 정규표현식
					}
